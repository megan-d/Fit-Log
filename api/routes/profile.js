...
//ROUTE: GET api/profile/me/demo
//DESCRIPTION: Get current demo profile
//ACCESS LEVEL: Public
router.get('/me/demo', async (req, res) => {
  try {
    //Find the current user based on the id that comes in with the request's token. Populate with the name from the user model.
    const profile = await Profile.findOne({ user: req.headers.userid }).populate(
      'User',
      'name',
    );
...
//DESCRIPTION: Create a demo user profile based on user's id
//ACCESS LEVEL: Private
router.post(
  '/demo',
    [
  ...
  async (req, res) => {
    ...
    profileItems.user = req.headers.userid;
    console.log(profileItems.user);
    if (weight) {
      profileItems.weight = weight;
      profileItems.weightHistory.weight = weight;
    }
    ...
    try {
      ...
      //Use findOne to find profile
      let profile = await Profile.findOne({ user: req.headers.userid });
